...
export async function webAssets({portable=false,output=null}={}){
 ...
 if(portable){
  ...
  auth='<script>window.LINKDID_PUBLIC_CONFIG='+JSON.stringify({url:process.env.SUPABASE_URL||'',key:process.env.SUPABASE_PUBLISHABLE_KEY||''}).replaceAll('<','\\u003c')+'</script><script defer src="'+hashed('auth.js',result.outputFiles[0].text,'application/javascript; charset=utf-8')+'"></script>';
 }
