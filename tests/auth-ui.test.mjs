...
test('Create account opens signup without submitting login; matching passwords required',async()=>{
 const s=await screen();try{
 s.q('[data-mode="signup"]').click();assert.equal(s.q('h1').textContent,'Create your account');assert.equal(s.calls.length,0);
 s.q('[name=email]').value='test@example.com';s.q('[name=password]').value='test-only-password';s.q('[name=confirm]').value='different-password';
 s.q('form').dispatchEvent(new s.w.Event('submit',{cancelable:true}));await s.settle();assert.match(s.q('#auth-message').textContent,/do not match/);assert.equal(s.calls.length,0);
 s.q('[name=confirm]').value='test-only-password';s.q('form').dispatchEvent(new s.w.Event('submit',{cancelable:true}));await s.settle();assert.equal(s.calls[0][0],'signup');assert.match(s.q('#auth-message').textContent,/confirm your email/);assert.equal(s.q('button[type=submit]').disabled,false);
 }finally{s.close()}
});
